...
class ServiceUser(AbstractUser):
    ...
    signature = models.ImageField(
        upload_to = 'ServiceUser/Signatures',
        default='ServiceUser/Signatures/Polygon.png',
        null=True,
        blank=True
        )
...
class ST1011_Permission(models.Model):
    user = models.OneToOneField(
        to=ServiceUser,
        related_name="ST1011_Permission",
        on_delete=models.CASCADE,
        null=True,
        blank=False,
    )
    ...
    guest = models.BooleanField(
        verbose_name="Guest Status",
        default=True,
        help_text="ST1011 permissions for Guests's access.",
    )
    registrar = models.BooleanField(
        verbose_name="Registrar Status",
        default=False,
        help_text="ST1011 permissions for Registrar's access.",
    )
    consultant = models.BooleanField(
        verbose_name="Consultant Status",
        default=False,
        help_text="ST1011 permissions for Consultant's access.",
    )
    clinician = models.BooleanField(
        verbose_name="Clinician Status",
        default=False,
        help_text="ST1011 permissions for Clinician's access.",
    )
    pathologist = models.BooleanField(
        verbose_name="Pathologist Status",
        default=False,
        help_text="ST1011 elevated permissions for Pathologist's access.",
    )
    ...
    class Meta:
        db_table = "ST1011 Permission"
        verbose_name_plural = "ST1011 Permissions"
